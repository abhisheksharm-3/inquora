...
export function parseGitHubUrl(url: string): TypeGitHubParseResult | null {
  const patterns = [/github\.com\/([^/]+)\/([^/\s?#]+)/i, /github\.com:([^/]+)\/([^/\s?#]+)/i];
 ...
 */
export async function storeDocsInPinecone(docs: Document[], namespace: string): Promise<void> {
  ...
  while (retries < GITHUB_MAX_RETRIES) {
    try {
      ...
      await new Promise((resolve) => setTimeout(resolve, GITHUB_RETRY_DELAY_MS * retries * 5));
    }
  }
}
