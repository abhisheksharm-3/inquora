...
const _scrapeWebPage = async (
  ...
  try {
    ...
    $(
      "script, style, nav, header, footer, aside, .navigation, .sidebar, .menu, .ad, .advertisement, .cookie, .popup, .modal, .overlay",
    ).remove();
    ...
    if (content.length < 100) {
      throw new Error(
        "Not enough meaningful content extracted from the page. The page might be empty, require authentication, or be heavily JavaScript-dependent.",
      );
    }
    ...
    console.log(
      `Successfully scraped page. Title: "${result.title}", Content length: ${content.length} characters, Domain: ${domain}`,
    );
    ...
    if (error instanceof Error) {
      if (error.name === "AbortError") {
        throw new Error(
          `Request timeout: The page took too long to load (>${config.timeout / 1000}s)`,
        );
      }
      if (error.message.includes("fetch")) {
        throw new Error(
          `Network error: Unable to connect to the website. The site might be down or blocking requests.`,
        );
      }
    }
...
export const getWebPageInfo = async (url: string): Promise<TypeWebPageInfo> => {
  ...
  try {
    ...
    throw new Error(
      `Failed to fetch page info: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
};
    ...
    const documents = validTexts.map((text, index) => {
      ...
      return new Document({
        ...
        metadata: {
          ...
          searchableText:
            `${pageContent.title} ${pageContent.metadata.description || ""} ${text}`.toLowerCase(),
        },
      });
    });
