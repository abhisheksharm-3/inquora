...
const _makeGitHubApiRequest = async (url: string, options: RequestInit = {}): Promise<any> => {
  ...
  if (!response.ok) {
    if (response.status === 403) {
      ...
      throw new Error(`GitHub API rate limit exceeded. Reset at: ${resetTime}, Remaining: ${remaining}`);
    }
    if (response.status === 404) {
      throw new Error(`Repository not found or is private. Make sure the repository exists and is accessible.`);
    }
 ...
 * @private
 */
const _fetchRepositoryInfo = async (owner: string, repo: string): Promise<TypeGitHubRepository> => {
 ...
 * @private
 */
const _fetchRepositoryTree = async (owner: string, repo: string, sha: string = 'HEAD'): Promise<TypeGitHubTreeItem[]> => {
 ...
 * @private
 */
const _fetchFileContent = async (owner: string, repo: string, path: string): Promise<string | null> => {
  try {
    ...
    if (response.content && response.encoding === 'base64') {
      ...
      console.log(`Successfully fetched ${content.length} characters from ${path}`);
      return content;
    }
 ...
 * @private
 */
const _filterProcessableFiles = (tree: TypeGitHubTreeItem[]): TypeGitHubTreeItem[] => {
  ...
  const processableFiles = tree.filter(item => {
    ...
    return PROCESSABLE_EXTENSIONS.has(extension) || PROCESSABLE_EXTENSIONS.has(fileName);
  });
    ...
    try {
      ...
      const document = new Document({
        ...
        metadata: {
          ...
          fileExtension: file.path.includes('.') ? '.' + file.path.split('.').pop() : '',
...
export const processGitHubRepository = async (
  ...
  console.log(`Starting GitHub repository processing for namespace: ${namespace}`);
  ...
  try {
    ...
    const tree = await _fetchRepositoryTree(owner, repo, repositoryInfo.default_branch);
    ...
    const documents = await _processRepositoryFiles(owner, repo, processableFiles, repositoryUrl);
