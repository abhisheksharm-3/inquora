...
export const syncMessagesWithOptimisticUpdates = (
  ...
  const finalOptimisticMessages = pendingOptimisticMessages.filter(
    (localMsg) => {
      if (
        localMsg.role === "assistant" &&
        localMsg.content === MessageConstants.AssistantThinkingContent
      ) {
        const hasRealAssistantResponse = serverMessages.some(
          (serverMsg) =>
