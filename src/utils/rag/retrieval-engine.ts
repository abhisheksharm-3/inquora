...
export async function retrieveRelevantDocuments(
  ...
  if (options.conversationHistory && options.conversationHistory.length > 0) {
    try {
      const contextualResults = await performContextualSearch(
        analysisResult,
...
async function performSemanticSearch(
  ...
  const documents = await queryDocuments(query, namespace, topK);
...
async function performKeywordSearch(
  ...
  const documents = await queryDocuments(keywordQuery, namespace, topK);
...
async function performContextualSearch(
  ...
  const documents = await queryDocuments(contextQuery, namespace, topK);
