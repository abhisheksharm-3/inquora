...
export function useSettings() {
  ...
  const stats = useMemo((): TypeSettingsStats | null => {
    ...
    const recentChats = chats.filter((chat) => new Date(chat.created_at) > sevenDaysAgo).length;

    const recentFiles = files.filter((file) => new Date(file.uploaded_at) > sevenDaysAgo).length;
